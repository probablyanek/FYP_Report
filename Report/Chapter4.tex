...
A standard CI calculates the best convex combination of two estimates that remain consistent with unknown cross correlations among parameters, where the best convex combination is parameterized by $\omega$ between 0 and 1~\cite{chen_ci,yang_heterogeneous}. An innovative aspect of the DCI algorithm is its replacement of static optimization of $\omega$ with a real-time computation of a freshness-based Age of Information penalty. The base variance for both sensors was empirically determined; radar variance was found to be 0.2~m$^2$ reflecting the tight near-field precision; Wi-Fi variance was determined to be 2.0~m$^2$ reflecting the larger multipath variability. The Wi-Fi covariance was modeled as time varying, increasing linearly with process noise over time relative to the last known position measurement~\cite{noroozi_aoi}:
